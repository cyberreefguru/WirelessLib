...
void setup() {
	...
	writeColor(0x0, true);
	colorWipe(1, LEFT, RED, BLUE, 25, 0, true, true);
	writeColor(0x0, true);
...
void parseCommand()
{
	...
	case CMD_PATTERN:
		...
		pattern(cmdPattern->pattern, cmdPattern->direction, cmdPattern->onColor, cmdPattern->offColor, cmdPattern->onTime, 0);
		break;
	case CMD_RUNWAY:
		printf("RUNWAY\n\r");
		dumpCommandRunway((uint8_t *)commandBuffer);
		cmdRunway = (runway_t *)commandBuffer;
		colorWipe(cmdRunway->pattern, cmdRunway->direction, cmdRunway->onColor, cmdRunway->offColor, cmdRunway->onTime, 0, cmdRunway->clearAfter, cmdRunway->clearEnd);
		break;
	case CMD_BOUNCE:
		printf("BOUNCE\n\r");
		cmdBounce = (bounce_t *)commandBuffer;
		dumpCommandBounce((uint8_t *)commandBuffer);
		colorBounce(cmdBounce->pattern, cmdBounce->direction, cmdBounce->onColor, cmdBounce->offColor, cmdBounce->onTime, 0, 0, cmdBounce->clearAfter, cmdBounce->clearEnd);
		break;
	case CMD_MIDDLE:
		printf("MIDDLE\n\r");
		cmdMiddle = (middle_t *)commandBuffer;
		middle(cmdMiddle->direction, cmdMiddle->onColor, cmdMiddle->offColor, cmdMiddle->onTime, cmdMiddle->offTime, cmdMiddle->clearAfter, cmdMiddle->clearEnd);
		break;
	case CMD_RAINBOW_FADE:
		printf("RAINBOW_FADE\n\r");
		cmdRainbowFade = (rainbow_fade_t *)commandBuffer;
		rainbowFadeStrip( cmdRainbowFade->fadeTime);
		break;
	case CMD_RAINBOW_FADE_ONE:
		printf("CMD_RAINBOW_FADE_ONE\n\r");
		cmdRainbowFadeOne = (rainbow_fade_one_t *)commandBuffer;
		rainbowFadeOne( cmdRainbowFadeOne->fadeTime);
		break;
	case CMD_RANDOM_FLASH:
		printf("RANDOM_FLASH\n\r");
		cmdRandomFlash = (random_flash_t *)commandBuffer;
		randomFlash( cmdRandomFlash->onTime, cmdRandomFlash->offTime);
		break;
	case CMD_RANDOM_FLASH_COLOR:
		printf("RANDOM_FLASH_COLOR\n\r");
		cmdRandomFlashColor = (random_flash_color_t *)commandBuffer;
		randomFlashColor( cmdRandomFlashColor->onTime, cmdRandomFlashColor->offTime, cmdRandomFlashColor->onColor, cmdRandomFlashColor->offColor );
		break;
	case CMD_SET_ALL:
		printf("SET_ALL\n\r");
		cmdSetAll = (set_all_t *)commandBuffer;
		writeColor( cmdSetAll->onColor, true );
		break;
	case CMD_ERROR:
	default:
		printf("ERROR\n\r");
